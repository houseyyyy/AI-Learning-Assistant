...
def main_process_generator(input_path: str,  doubao_app_id: str, doubao_token: str,dify_api_key: str, output_filename: str, query: str):
    """
    ...
    # === 视频和音频文件工作流 ===
        ...
        try:
            ...
            user_friendly_error = f"**音频转录失败**\n\n在连接豆包语音识别服务进行语音转文字时发生无法恢复的错误。\n\n**可能原因:**\n1. **豆包服务中断**: 可前往其官网查看服务状态。\n2. **网络连接问题**: 您的服务器可能无法访问 豆包 API。\n3. **音频数据问题**: 某个音频块可能已损坏无法处理。\n\n**原始错误信息:**\n`{e}`"
            yield "persistent_error", 0, user_friendly_error
            return
